...
import { rechercher_recu_deposer_par_reference, rechercher_reçu_deposant } from '../../../store/reçuSlice'


export default function Liste_des_reçus_deposer(props) {
    ...
    const onKeyDown = async (e) => {
        ...
        if (e.target.value === "") { dispatch(rechercher_reçu_deposant(props.deposant._id)).then(async (action) => { props.setReçus(action.payload.reçu) }) }
        else {
            dispatch(rechercher_recu_deposer_par_reference(e.target.value)).then(async (action) => {
                if (action.payload.reçu.length > 0) {
                    props.setReçus(action.payload.reçu)
                    setMessage("")
                }
                else {
                    dispatch(rechercher_reçu_deposant(props.deposant._id)).then(async (action) => { props.setReçus(action.payload.reçu) })
    ...
    return (
        <div>
                                        ...
                                        {props.reçus?.map((reçu, index) =>
                                            <tr key={index} >
                                                <td className="text-sm text-primary text-center font-weight-bold" >
                                                    <a data-bs-toggle="modal" data-bs-target="#recus" type="button" onClick={() => modal(reçu)}>
            ...
            <div className="modal fade" id="recus" tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
